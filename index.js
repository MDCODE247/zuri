...
function handleNum(event) {
    ...
    if (displayContent.length >= 8) {
        display.setAttribute(
            "style",
            "border-color: #8b0000; transform: scale(1.02);"
        );
        return;
    }
...
function handleOp(event) {
    ...
    if (operator === "") {
        ...
        if (isOperator(lastChar)) {
            display.textContent =
                display.textContent.slice(0, -1) + event.target.textContent;
            operator = event.target.textContent;
        } else {
            operate();
            display.setAttribute("style", "border-color: #333333;");
...
function inputDecimal() {
    if (display.textContent.includes(".") && firstNum.includes(".")) {
        return;
    }
    if (firstNum.includes(".") && lastNum.includes(".")) {
        return;
    }
    if (display.textContent == 0) {
        display.textContent = "";
